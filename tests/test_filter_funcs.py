...
def test_isolated_noise_filter():
    ...
    filtered_evs = isolated_noise_filter(evs, spatial_radius=5, time_window=10, min_neighbors=2)

    assert len(filtered_evs) == 4
    assert all((50, 50, 2000, -1) != (ev['x'], ev['y'], ev['t'], ev['p']) for ev in filtered_evs)
